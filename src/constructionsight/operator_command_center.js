...
function ingestionInboxMarkup(inbox) {
  if(!inbox || inbox.read_only!==true || inbox.network_executed!==false ||
    inbox.persistence_mutated!==false || !Number.isSafeInteger(inbox.candidate_count) ||
    !Number.isSafeInteger(inbox.pending_capture_count) ||
    !Number.isSafeInteger(inbox.persisted_candidate_count) ||
    !Number.isSafeInteger(inbox.conflict_candidate_count) ||
    !Array.isArray(inbox.candidates) ||
    inbox.candidate_count!==inbox.candidates.length ||
    inbox.pending_capture_count+inbox.persisted_candidate_count+inbox.conflict_candidate_count!==inbox.candidate_count)
  ...
  if(inbox.configured!==true)
    return '<section class="feature-card"><span class="badge">CEQANET INGESTION INBOX · NOT CONFIGURED</span>'+
      '<h2>Discovery queue status</h2><p>Start the local operator with both <code>--ceqanet-listing-evidence</code> and '+
      '<code>--ceqanet-queue-evidence</code> to reconcile one immutable reviewed queue against the selected SQLite database. '+
      'No network request or write is performed by the dashboard.</p></section>';
  ...
  const next=inbox.next_pending_sch ?
    ...
    '. Use <code>constructionsight-ceqanet-reviewed-import capture-next-preview</code> with the same exact listing, queue and database. '+
    'The resulting source and plan digests still require the separate reviewed <code>apply</code> authorization.</p>' :
    '<p>No pending reviewed SCH is available in this configured queue. Conflicts, if any, require evidence review rather than another automatic capture.</p>';
  return '<section class="feature-card"><span class="badge">CEQANET INGESTION INBOX · READ ONLY</span>'+
    ...
    inbox.pending_capture_count+' pending capture · '+inbox.persisted_candidate_count+' persisted source-claim matches · '+
    inbox.conflict_candidate_count+' conflicts.</p>'+next+
    ...
    rows+'</tbody></table></div><p>No browser request, SQLite mutation, lead qualification, outreach or bid action is authorized by this status view.</p></section>';
}
