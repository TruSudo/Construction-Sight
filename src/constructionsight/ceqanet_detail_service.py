...
def execute_authorized_ceqanet_detail(
    *,
    ...
    if timeout_seconds <= 0 or timeout_seconds > CEQANET_DETAIL_POLICY.read_timeout_seconds:
        raise ValueError("timeout_seconds cannot exceed the declared CS-NET-006 ceiling")
    if max_body_bytes < 1 or max_body_bytes > CEQANET_DETAIL_POLICY.max_response_bytes:
        raise ValueError("max_body_bytes cannot exceed the declared CS-NET-006 ceiling")
    ...
    return {
        "metadata": {
            ...
            "allowed": True,
            "reason": (
                "CEQAnet detail executor completed one authorized bounded read-only GET request."
            ),
