...
class IntelligenceGraphService:
    ...
    def __init__(
        self, store: IntelligenceStore, *, source_service: str = "intelligence_graph_service"
    ) -> None:
        self.store = store
    ...
    def upsert_entity(self, entity: EntityIdentity) -> EntityIdentity:
        ...
        self._emit_event(
            ...
            event_type=RuntimeEventType.ENTITY_MERGED
            if was_existing
    ...
    def upsert_relationship(self, relationship: RelationshipAssertion) -> RelationshipAssertion:
        ...
        self._emit_event(
            ...
            event_type=RuntimeEventType.RELATIONSHIP_UPDATED
            if was_existing
            else RuntimeEventType.RELATIONSHIP_CREATED,
            severity=RuntimeEventSeverity.HIGH,
            message=(
                f"Relationship {'updated' if was_existing else 'recorded'}: "
    ...
    def upsert_project_cluster(self, cluster: ProjectCluster) -> ProjectCluster:
        ...
        self._emit_event(
            ...
            event_type=RuntimeEventType.PROJECT_CLUSTER_UPDATED
            if was_existing
            else RuntimeEventType.PROJECT_CLUSTER_CREATED,
            severity=RuntimeEventSeverity.HIGH,
            message=(
                f"Project cluster {'updated' if was_existing else 'recorded'}: "
                f"{cluster.project_name or cluster.project_cluster_id}."
            ),
    ...
    def upsert_opportunity(self, opportunity: OpportunitySignal) -> OpportunitySignal:
        ...
        self._emit_event(
            ...
            severity=RuntimeEventSeverity.MEDIUM,
            message=(
                f"Opportunity signal {'updated' if was_existing else 'recorded'}: "
                f"{opportunity.category.value}."
            ),
            project_cluster_refs=[opportunity.project_cluster_id]
            if opportunity.project_cluster_id
            else [],
