...
def _signal() -> OpportunityEnrichmentSignal:
...
def _duplicate_result() -> LeadDuplicateResult:
    return LeadDuplicateResult(
        ...
        limitations=["review prior outreach history"],
    )
...
def test_store_opportunity_enrichment_report_roundtrip() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert row.report_id == "opportunity-enrichment:test"
        assert row.confidence_band == "probable"
...
def test_store_lead_review_package_roundtrip() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert row.package_id == "lead-review:test"
...
def test_store_lead_fingerprint_roundtrip() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert row.fingerprint_key == "lead-fingerprint:test"
...
def test_store_lead_duplicate_result_roundtrip() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert row.result_id == "lead-duplicate:test"
        assert row.status == "review_needed"
        assert row.matched_count == 1
        payload = json.loads(row.payload_json)
        assert payload["limitations"] == ["review prior outreach history"]
...
def test_store_lead_workflow_record_roundtrip_stores_events() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert workflow_row.workflow_id == "lead-workflow:test"
...
def test_store_result_share_record_roundtrip() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert row.share_record_id == "result-share:test"
...
def test_store_result_ledger_record_roundtrip_stores_share() -> None:
    ...
    with managed_session(factory) as session:
        ...
        assert ledger_row.ledger_id == "result-ledger:test"
...
def test_store_helpers_update_existing_rows() -> None:
    _engine, factory = _session_factory()
    first = _fingerprint(score=20)
    second = _fingerprint(score=80)

    with managed_session(factory) as session:
        store_lead_fingerprint(session, first)
        store_lead_fingerprint(session, second)
        session.flush()
