...
The command uses the preexisting, effect-consumed authorization layer to execute **one exact public GET**, preserving the complete source-execution JSON through the protected, no-overwrite runtime artifact writer **before** the offline bridge parses it. If verification or bridging fails, retained evidence stays available for investigation but there is **no proposed import plan**. The existing durable exact-request effect layer may legitimately return an earlier, previously authorized capture for the same SCH. This command checks that the execution timestamp falls within the present invocation; an older replay is preserved for audit and **rejected as a newly acquired observation without preparing an import plan**. Consequently this single-request workflow cannot yet re-query a previously consumed exact SCH under a new recurring-update allowance. It is a bounded first-capture path for newly selected SCH projects, **not a fully implemented recurring current-data refresh system**. The successful console response includes the within-invocation execution-timestamp check, execution timestamp, original source SHA-256, exact proposed write-plan digest, source-record identities and count, and the paths of the retained artifact and optional unapproved plan. It explicitly reports `persistence_mutated: false` and `source_claims_verified: false`.
