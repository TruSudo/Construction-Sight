...
| CS-PLAN-002 | Verified recurring live adapters | CEQAnet has `partial` maturity and report-only proposal tooling that binds the registry, execution, body, replay, and inspection digests while deterministically keeping the source partial. HTML automation remains blocked by HTTP 403; no scheduler exists. | Review and approve a separate bounded official-CSV access policy, then collect a governed multi-run evidence series before any promotion. |
