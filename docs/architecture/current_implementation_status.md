...
| Parcel ArcGIS acquisition gates | Yes | Yes | Yes | Yes | Yes | Bounded on demand | Exact capability snapshots, four-request probe plans, immutable response observations, complete-rehearsal manifests, and derived assessments separate advertised from executed proof. Both official sources remain `metadata_only`; bulk authorization and automatic promotion are impossible. |
