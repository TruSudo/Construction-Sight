...
| CS-VIAM-003 | P1 | Source maturity | `source_status_report.py`, `source_status_cli.py`, `source_readiness_*`, `data/source_registry.seed.json` | Seed/status/readiness reporting now separates registry claims, adapter maturity, reachability evidence, and verified usable source status. | Product must not overclaim source readiness. | Source records and adapter contracts previously had separate audits but no report-only readiness workflow. | Keep source readiness report-only unless an explicit dry-run-first registry update workflow is added later. | Fixed; monitor forward. |
| CS-VIAM-004 | P1 | Adapter maturity | `src/constructionsight/adapters/specs.py` | Most adapter families are placeholders; CEQAnet is contract-ready, not live production coverage. | Avoid false live-integration claims. | Adapter contracts preceded live adapters. | Use source-status and source-readiness reports before making coverage claims. | Partially fixed; live maturity still backlog. |
...
| CS-VIAM-009 | P2 | CLI | `pyproject.toml` | Persisted layers now have storage-summary, source-status, and source-readiness CLIs, but dedicated action/list/detail CLIs are still pending. | Operators can verify storage shape/counts and source maturity/readiness, but cannot yet operate every persisted domain layer directly. | Model/service/storage-first implementation. | Add consolidated lead/parcel operator CLIs after transition and source maturity rules are hardened. | Partially fixed. |
| CS-VIAM-010 | P2 | Documentation | `docs/architecture/*` | Lead dedupe, workflow, result ledger, source-status, and source-readiness docs now exist or are represented in the status matrix. | Status matrix must not remain stale after documentation lands. | Rapid implementation outpaced matrix reconciliation. | Keep status matrix synchronized with documentation/runtime. | Fixed. |
