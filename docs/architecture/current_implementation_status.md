...
| Result ledger/share calculation | Yes | Yes | Yes | Yes | Yes | No | Ledger/share list and detail access preserve explicit share states. Ledger and share rows are immutable and content-addressed for new canonical results. |
| Result ledger authority | Yes | Yes | Yes | Yes | No | No | One current authority pointer per workflow, append-only correction events, exact expected-state checks, workflow/outcome compatibility, and atomic compare-and-swap persistence exist. Operator apply and authority-history commands remain pending. |
...
| CS-VIAM-012 | P1 | Result ledger authority | `result_ledger_authority_*`, `result_ledger_service.py`, `storage/lead_workflow_store.py` | Multiple immutable result assertions can exist for one workflow, but one authoritative current outcome must be explicit and correction history must not overwrite prior rows. | Without authority doctrine, differing won/lost/no-fit rows could appear equally current. | Result modeling preceded correction and supersession governance. | Use a single current pointer, append-only authority events, content-addressed immutable ledgers, exact expected-current checks, and atomic revision compare-and-swap. | Fixed at service/persistence layer; operator command pending. |
...
| CS-VIAM-014 | P2 | Result authority operator access | `lead_operator_*`, `result_ledger_authority_*` | Authority service and persistence exist, but current authority, correction events, and apply behavior are not yet exposed through the lead operator CLI. | Operators need an auditable governed command rather than direct service or database access. | Authority foundation was implemented before its operator surface. | Add read-only authority/history views and an explicit apply command requiring expected current ledger, reason, and authorization. | Open; next operator phase. |
