...
Current active-defect count: **0**. The authoritative ledger is `governance/active_defects.toml`. CS-SR-001 through CS-SR-096 are historical resolved-defect evidence; no active defect is represented as closed solely by prose. The corrected tree still requires a fresh five-pass exact-tree Native Maximum assurance cycle before final release certification.
