...
| Permit snapshot transition spine | Yes | Yes | Yes | No | No | No | First source-neutral permit movement model exists; persistence and adapters pending. |
| Contractor identity | Yes | Yes | Yes | No | No | No | Model/service exists; CSLB live verification pending. |
| Decision record | Yes | Yes | Yes | No | No | No | Pre-permit decision model exists; agenda/staff-report adapters pending. |
...
| Lead dedupe | Yes | Yes | No | No | No | No | Code/tests exist; architecture doc still needed. |
| Lead workflow status | Yes | Yes | No | No | No | No | Status/event model exists; transition matrix hardening pending. |
| Result ledger/share calculation | Yes | Yes | No | No | No | No | Ledger/share model exists; business semantics for pending share need review. |
...
|---|---|---|---|---|---|---|---|---|
| CS-VIAM-001 | P1 | Doctrine | `README.md` | README stops at parcel core record and does not reflect PRs #50-#58. | Runtime and doctrine drift apart. | Rapid implementation outpaced docs. | Update README with all implemented layers and maturity warnings. | Fix now. |
| CS-VIAM-002 | P1 | Persistence | `src/constructionsight/storage/*` | New model/service layers after parcel core are not represented in ORM/store coverage. | Models cannot be operationally stored or queried. | Architecture-first implementation sequence. | Add storage coverage matrix, then ORM/store PRs. | Backlog Phase 2-4. |
...
| CS-VIAM-004 | P1 | Adapter maturity | `src/constructionsight/adapters/specs.py` | Most adapter families are placeholders; CEQAnet is contract-ready, not live. | Avoid false live-integration claims. | Adapter contracts preceded live adapters. | Add maturity language to README/status doc. | Fix docs now. |
...
| CS-VIAM-010 | P2 | Documentation | `docs/architecture/*` | Lead dedupe, workflow, and result ledger docs are missing or incomplete. | Later contributors lose context. | Some doc writes were blocked during rapid PR work. | Add neutral docs in this cleanup phase. | Fix now. |
