...
| Result ledger/share calculation | Yes | Yes | Yes | Yes | Read-only | No | Ledger/share list and detail access preserve explicit share states. Append-only revision and supersession doctrine now provides one validated authoritative tip while preserving every prior outcome payload; operator mutation remains withheld pending a dedicated stale-state-protected action. |
...
| CS-VIAM-008 | P1 | Result ledger | `result_ledger_models.py`, `result_ledger_service.py` | Won results can explicitly distinguish pending gross value, pending share rate, and calculated share, and every correction is represented as a new immutable revision. | Pending-share uncertainty and outcome correction history remain queryable without overwriting earlier evidence. | Share-rate uncertainty and correction authority were initially modeled separately. | Preserve linear supersession, immutable payloads, correction reasons, and explicit share state in future operator actions. | Fixed; monitor forward. |
...
| CS-VIAM-012 | P1 | Result ledger authority | `result_ledger_models.py`, `result_ledger_service.py`, `storage/lead_workflow_store.py` | Result histories now require one root, contiguous immutable revisions, explicit predecessor links, correction reasons, and one unbranched authoritative tip. | Differing outcomes can be corrected without concurrent authority or destruction of prior evidence. | Result modeling preceded correction/history governance. | Keep persistence append-only and require stale-state checks before exposing a future operator correction command. | Fixed at model/service/persistence boundary; operator correction action remains controlled backlog. |
