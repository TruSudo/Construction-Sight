...
| Source audit package | Yes | Yes | Yes | No | Yes | No | Report-only audit package preserves original/final URLs, HTTP status, redirect classification, source/adapter/readiness state, reasons, limitations, and next action before any future registry update workflow. |
...
| Source plan reports | Yes | Yes | Yes | No | Yes | No | Dry-run source plan commands can produce review actions and proposed registry payloads without writing registry data or creating verified coverage. |
...
| CS-VIAM-003 | P1 | Source maturity | `source_status_report.py`, `source_status_cli.py`, `source_readiness_*`, `audit_package_cli.py`, `source_verification_*`, `source_promotion_plan_*`, `source_registry_update_plan_*`, `data/source_registry.seed.json` | Seed/status/readiness/audit-package/checklist/source-plan reporting now separates registry claims, adapter maturity, reachability evidence, redirect evidence, manual observations, dry-run actions, proposed payloads, and verified usable source status. | Product must not overclaim source readiness. | Source records and adapter contracts previously had no layered report-only source maturity workflow. | Keep source maturity workflows report-only unless an explicit apply workflow is added later. | Fixed; monitor forward. |
| CS-VIAM-004 | P1 | Adapter maturity | `src/constructionsight/adapters/specs.py` | Most adapter families are placeholders; CEQAnet is contract-ready, not live production coverage. | Avoid false live-integration claims. | Adapter contracts preceded live adapters. | Use source-status, source-readiness, audit-package, checklist, and source-plan reports before making coverage claims. | Partially fixed; live maturity still backlog. |
...
| CS-VIAM-009 | P2 | CLI | `pyproject.toml` | Persisted layers now have storage-summary, source-status, source-readiness, audit-package, source-checklist, source-promotion, and source-plan CLIs, but dedicated action/list/detail CLIs are still pending. | Operators can verify storage shape/counts and source maturity evidence, but cannot yet operate every persisted domain layer directly. | Model/service/storage-first implementation. | Add consolidated lead/parcel operator CLIs after transition and source maturity rules are hardened. | Partially fixed. |
| CS-VIAM-010 | P2 | Documentation | `docs/architecture/*` | Lead dedupe, workflow, result ledger, source-status, source-readiness, audit-package, source-checklist, source-promotion, and source-plan docs now exist or are represented in the status matrix. | Status matrix must not remain stale after documentation lands. | Rapid implementation outpaced matrix reconciliation. | Keep status matrix synchronized with documentation/runtime. | Fixed. |
