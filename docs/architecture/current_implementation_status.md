...
| CS-VIAM-003 | P1 | Source maturity | `source_status_report.py`, `source_status_cli.py`, `source_readiness_*`, `audit_package_cli.py`, `source_verification_*`, `data/source_registry.seed.json` | Seed/status/readiness/audit-package/checklist reporting now separates registry claims, adapter maturity, reachability evidence, redirect evidence, manual observations, and verified usable source status. | Product must not overclaim source readiness. | Source records and adapter contracts previously had no layered report-only source maturity workflow. | Keep source maturity workflows report-only unless an explicit dry-run-first registry update workflow is added later. | Fixed; monitor forward. |
| CS-VIAM-004 | P1 | Adapter maturity | `src/constructionsight/adapters/specs.py` | Most adapter families are placeholders; CEQAnet is contract-ready, not live production coverage. | Avoid false live-integration claims. | Adapter contracts preceded live adapters. | Use source-status, source-readiness, audit-package, and checklist reports before making coverage claims. | Partially fixed; live maturity still backlog. |
