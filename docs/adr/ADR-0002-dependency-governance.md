...
CI installs **only** the selected exact artifact-hashed third-party lock with `--require-hashes`, `--only-binary=:all:`, and `--no-deps`. The untrusted candidate project is **not installed or built** before quality and governance gates; the canonical checked-out source is loaded through the reviewed `PYTHONPATH` binding. The source project identity is checked without running the candidate build backend, and installed-project shadowing is rejected. The supply-chain certifier rejects unhashed, nonexact, duplicate, malformed, or dangling lock entries, verifies the declared exact environment and source project identity, runs `pip check`, performs a fatal vulnerability audit, and emits a deterministic SBOM. This source-tree CI procedure does not certify the installability or execution of a distributed ConstructionSight wheel.
