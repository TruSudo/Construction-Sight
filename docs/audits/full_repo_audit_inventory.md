...
- digest-bound ArcGIS capability, retained county bounded proofs, exact-response rehearsal execution and HTTP planning, portable independent verification, and expiring single-use exact-plan authorization/preflight with downstream authority fixed false;
...
| Parcel ArcGIS acquisition gates | Yes | Yes | Yes | Yes | Read-only plus explicit bounded probe and guarded proof persistence | Bounded on demand | Retained county bundles remain `bounded_query_verified`. The executor, HTTP adapter, portable verifier, and exact-plan single-use authorization/preflight contract exist, but no authorization artifact, durable consumption ledger, live county rehearsal, import, profile promotion, recurring authority, or production bulk run exists. |
...
Current active entries: **38 (`CS-SR-001` through `CS-SR-038`)**. The August 13 adversarial review invalidated candidate `4cc3ce763354ea91ef8c4bed28c3ce0b531379c3` and added eight P0 findings as `CS-SR-028` through `CS-SR-035`; exact-head CI #1056 later discovered newly published pip advisory `PYSEC-2026-3721` and registered `CS-SR-036`; corrected-candidate adversarial verification then exposed `CS-SR-037` against `1296b6053da3a5f30cbed702cc1333e9365bb307`; after CI #1062 accepted that correction's ordinary gates, continued review exposed `CS-SR-038` against `ffaa745dea866dadc3d21e0272c6361128a80901`. The first 27 entries retain their prior correction evidence but remain open pending legitimate closure. Correction and regression evidence exists for `CS-SR-028` through `CS-SR-036`; the `CS-SR-037` correction uses an explicit request, disables injected client authentication, rejects request hooks before execution, and adds focused negative regressions. the working `CS-SR-038` correction rejects response hooks before transport execution and includes a focused negative regression and mutant; candidates `1296b6053da3a5f30cbed702cc1333e9365bb307` and `ffaa745dea866dadc3d21e0272c6361128a80901` remain invalidated. Every entry remains active until fresh independent review and exact-tree certification permit legitimate closure. CI remains the authoritative executable check for the exact tree.
...
| CS-AUDIT-028 | P1 | ArcGIS bulk rehearsal proof | The implemented bulk manifest accepted bare checkpoint/resume and retry booleans plus opaque digests, so a caller could claim complete-rehearsal maturity without retaining evidence capable of substantiating those assertions. | Structured digest-bound page, checkpoint, resume, and retry records are now mandatory; every retried page requires a matching recovery record, all proof timestamps are bound to the manifest execution window, and manifest summaries are derived from and revalidated against the complete embedded evidence chain. |
| CS-AUDIT-029 | P1 | ArcGIS rehearsal response retention | A complete-rehearsal executor could derive response digests from parsed mappings without preserving the exact successful count and page bodies required to substantiate those digests independently. | Count/page response envelopes now retain exact bytes; digest-addressed atomic artifacts cover both counts and every successful page; contiguous receipts are required; and page artifact digests must equal the manifest proof. |
...
| Parcel ArcGIS acquisition gates | Six additive tables preserve capability snapshots, bounded plans, one immutable observation per request, complete-rehearsal manifests, assessments, and self-contained bounded proof bundles with dependency-ordered writes and typed digest revalidation. Bulk manifests embed structured page, checkpoint, resume, and retry evidence. The reusable rehearsal executor additionally uses atomic filesystem stores for exact count/page response bytes and checkpoint reload proof; the digest-bound HTTP plan and transport are ephemeral control objects and create no operational county data or scheduler state. Canonical bounded bundles, verification reports, and persistence receipts remain committed for both counties. |
...
| Parcel ArcGIS acquisition | Static capability/plan/readiness commands, an explicit bounded live probe, offline bundle verification, exact-identity persistence, six read-only upstream record kinds, and storage summaries are present. A reusable complete-rehearsal executor plus digest-bound official HTTP plan/adapter retain exact response artifacts and checkpoint/retry/count-order proof under mocked tests. No live county rehearsal command, profile-promotion command, complete county import, or recurring acquisition exists. |
...
- ArcGIS metadata projection and schema digests, object-ID uniqueness, query primitives, safe request identity, digest-bound rehearsal-plan identity, exact HTTPS query endpoint and parameters, timeout/size/media-type/retry controls, exact bounded-response digests, count/page/replay sequence, exact complete-rehearsal count and page response bytes, artifact receipts and references, count reconciliation, duplicate and failed-page counts, checkpoint save/reload and retry proof, terminal-page evidence, object-ID-set digest, acquisition status, and explicit gaps;
