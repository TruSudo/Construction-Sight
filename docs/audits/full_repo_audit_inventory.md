...
- CEQAnet source/evidence-bound recurring-run definitions, exact-window manifests, explicit attempts, and execution verification;
...
The active-defect count for a certified tree must be zero. Planned capabilities are not counted as defects when their absence is explicit, guarded, non-overclaiming, and recorded with an entry condition.
...
| CEQAnet recurring-run governance | Yes | Yes | Yes | Artifact-only | Yes | Guarded only | Definition and manifest digests bind source identity, evidence, query, window, and limits. Execution requires current registry/checklist agreement and explicit authorization. Canonical source data remains blocked because it is unverified. |
...
| CS-PLAN-002 | Verified recurring live source adapters | CEQAnet has evidence-bound definitions, exact-window manifests, stale-evidence rejection, explicit attempts, and execution verification. Canonical source data remains unverified; attempts are not scheduled or persisted as a ledger. | Complete source-specific evidence review and maturity promotion, then define persisted attempt identity, scheduling, retry, archive, observability, and downstream handoff governance. |
...
| CEQAnet recurring-run definitions/manifests/executions | Explicit JSON artifacts only; no attempt ledger or scheduler state is implied. |
...
| CEQAnet recurring-run artifacts and attempts | Definition, manifest, execute, and verify commands exist; canonical source data remains blocked. |
...
- source registry/checklist digests, recurring-run definition/manifest identities, exact windows, attempt identity, and stale-evidence findings;
