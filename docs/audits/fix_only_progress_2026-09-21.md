...
## Second batch committed and qualified against its exact heads

Hardening commit `05c328d465ed30f96b9c165444b22ddebb2a279d` (tree `43e7b7b12a6086654873c04d18adb7b56f409d1f`) and GUI merge `d5dfa7551e8c447e00a6a21941d0a91c56a959b6` (tree `fca3ac9419526345f15878077ec95a67e0dabb9a`) were published without force after clean exact-commit structural preflight. Both PRs remain drafts with the same bases.

Actual logs for [hardening run 35593180336](https://github.com/TruSudo/Construction-Sight/actions/runs/35593180336) and [GUI run 35593181100](https://github.com/TruSudo/Construction-Sight/actions/runs/35593181100) show 1,601 hardening / 1,705 GUI tests and **127/127 mutation witnesses killed on both Python 3.11 and 3.12**. Ruff, strict mypy, dependency controls, structural preflight and isolated vulnerability audits passed. The quality gate's only unsuccessful mandatory command was release certification, with the same 75 active defects and missing assurance report. Original logs are retained in the following batch's evidence manifest.

## Third correction batch: all operator text loaders and byte-snapshot binding

Starting from those exact heads, 121 new failing regression cases reproduced final and ancestor symlink traversal, unbounded JSON/HTML materialization, and replacement between HTML format detection and extraction. Corrections route operator JSON and stored HTML through one shared contained UTF-8 reader with a 16-MiB ceiling, including source registries, observations, plans, retained execution/replay evidence, parcel previews, and intake records. Raw CSV retains its tighter 10,000,000-byte ceiling and universal intake retains its existing 16-MiB limit; both now use the no-follow reader. Existing user-facing size errors are preserved through a typed limit exception.

Detail, listing-result and vocabulary HTML extraction now consumes the same captured text used for format detection. The test replaces the file during detection and asserts that extraction returns the original bytes. Nine additional replay tests independently exercise the source and replay input paths. All 130 new cases pass. A full read-call audit found no remaining ordinary runtime `read_text`/`read_bytes` call in the hardening source; governance readers and the archive staging reader are separate remaining/trusted scopes. GUI static assets are repository-owned presentation resources.
